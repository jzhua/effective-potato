...
class EcommerceDataGenerator:
    ...
    def generate_product_name(self) -> str:
        base_product = random.choice(list(self.product_variations.keys()))
    ...
    def generate_quantity(self, product_name: str = "", category: str = "") -> str:
        """Generate realistic quantities based on product type."""
        if self.clean_data:
            # Clean quantities based on product type
            product_lower = product_name.lower()
            
            # Single-item products (typically bought one at a time)
            if any(term in product_lower for term in ['laptop', 'phone', 'tv', 'sofa', 'refrigerator', 'car']):
                return str(1)
            
            # Small quantities for expensive items
            elif any(term in product_lower for term in ['jewelry', 'watch', 'camera', 'tablet']):
                return str(random.randint(1, 2))
            
            # Consumables and small items (higher quantities)
            elif any(term in product_lower for term in ['food', 'snack', 'pen', 'paper', 'battery']):
                weights = [0.3, 0.25, 0.2, 0.15, 0.1]  # Favor smaller quantities
                return str(random.choices(range(1, 6), weights=weights)[0])
            
            # Clothing items
            elif any(term in product_lower for term in ['shirt', 'pants', 'shoes', 'dress', 'jacket']):
                return str(random.choices([1, 2, 3], weights=[0.7, 0.2, 0.1])[0])
                
            # Default for other items
            else:
                return str(random.choices([1, 2, 3, 4, 5], weights=[0.5, 0.25, 0.15, 0.07, 0.03])[0])
        
        # Dirty data generation
        rand = random.random()
        if rand < 0.85:
            # Realistic quantities with weighted distribution
            product_lower = product_name.lower()
            
            if any(term in product_lower for term in ['laptop', 'phone', 'tv', 'refrigerator']):
                return str(random.choices([1, 2], weights=[0.9, 0.1])[0])
            elif any(term in product_lower for term in ['food', 'snack', 'battery']):
                return str(random.choices(range(1, 11), weights=[0.2, 0.18, 0.15, 0.12, 0.1, 0.08, 0.07, 0.05, 0.03, 0.02])[0])
            else:
                return str(random.choices(range(1, 8), weights=[0.4, 0.25, 0.15, 0.1, 0.05, 0.03, 0.02])[0])
        elif rand < 0.95:
            # Edge cases: 0, negative, or large numbers
            return str(random.choice([0, -1, -5, 50, 100]))
        else:
            # String values that should be numbers
            return random.choice(["two", "3.5", "many", "N/A", ""])

    def generate_unit_price(self, product_name: str = "", category: str = "") -> float:
        """Generate realistic prices based on product category and type."""
        
        # Get base price range from shared constants
        min_price, max_price = CATEGORY_PRICE_RANGES.get(category, (10, 500))
        
        # Adjust prices based on specific product types
        product_lower = product_name.lower()
    ...
    def generate_row(self) -> Dict[str, Any]:
        ...
        quantity = self.generate_quantity(product_name, category)
        unit_price = self.generate_unit_price(product_name, category)
    ...
    def generate_csv(self, filename: str, num_rows: int = 1000):
        ...
        print(f"Generated {num_rows} rows of e-commerce data in '{filename}'")
...
if __name__ == "__main__":
    ...
    print("Generating test sample with data errors...")
    generator.generate_csv("test_sample.csv", 5000)
    
    # Generate clean data sample
    print("Generating clean data sample...")
    ...
    print("\nSample product names with data errors:")
    for i in range(10):
        product = generator.generate_product_name()
        print(f"- {product}")

    print("\nTo generate full dataset, uncomment the line below:")
    print("# generator.generate_csv('ecommerce_data_100m.csv', 100000000)")

    print("\nSample of data quality issues you'll find:")
    print("- Duplicate order IDs (~5% chance)")
    print("- Product name variations (iPhone vs iphone vs I-Phone)")
    print("- Product name spelling errors (~15% chance): iPhoen, Samsng, MacBok")
    print("- Data corruption: missing chars, extra spaces, punctuation")
    print("- Inconsistent categories (Electronics vs electronics)")
    print("- Invalid quantities (negative, zero, strings like 'two')")
    print("- Discount percentages > 1 or negative")
    print("- Region spelling mistakes (Eurpoe instead of Europe)")
    print("- Mixed date formats and null dates")
    print("- Missing or malformed email addresses")
